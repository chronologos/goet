...
// CanCatchup reports whether the buffer contains data after the given
// sequence number. If afterSeq is older than the oldest stored entry,
// the client will get a partial catchup (some output was lost).
func (b *Buffer) CanCatchup(afterSeq uint64) (full bool, partial bool) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	oldestSeq := b.entries[b.tail()].seq
	newestSeq := b.entries[b.newest()].seq
	...
	// afterSeq+1 is the first seq the client needs
	if afterSeq+1 >= oldestSeq {
		return true, false
	}

	// Client needs data older than our oldest — partial catchup
	return false, true
}
